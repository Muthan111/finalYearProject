...
class WhisperService:
    ...
    async def transcribe(self, audio_file):
        logger.info(f"[transcribe function] Transcribing audio file (use_gradio = {self.use_gradio})")
        await asyncio.sleep(0)
        if self.use_gradio:
            try:
                return self.transcribe_gradio(audio_file)
            except Exception as e:
                logger.error(f"[transcribe function] Gradio transcription failed: {e}")
                logger.error(traceback.format_exc())
                raise HTTPException(status_code=500, detail="Gradio transcription failed")
            
        else:
            try:
                return await self.transcribe_using_local_model(audio_file)
            except Exception as e:
                logger.error(f"[transcribe function] Local transcription failed: {e}")
                logger.error(traceback.format_exc())
