...
class UploadService:
    ...
    It saves the uploaded files to a specific directory and returns the
    file path and display URL.
    ...
    def audio_upload(self, file):
        ...
        Returns:
            A dictionary containing the filename,
            file path, and display URL if successful.
            An error message if an exception occurs.
        """
        try:
            logger.info(
                f"{self.audio_upload.__name__}Starting audio upload process."
            )
            ...
            if size > self.MAX_FILE_SIZE:
                raise ValueError(f"""File size {size / (1024 * 1024):.2f}MB
                    exceeds limit of {self.MAX_FILE_SIZE / (1024 * 1024)}MB""")
            ...
            logger.info(f"""✅ File '{safe_filename}'
                  uploaded successfully to '{self.directory}'""")
            return {
