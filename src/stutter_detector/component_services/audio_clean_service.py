...
class AudioCleanService:
    ...
    def save_processed_audio(self, audio, input_name, file_path):
        """
        Saves the processed audio to a
        specified directory with a unique filename.
        Args:
        ...
        Returns:
            ...
            the audio ID, filename, and full path of the saved audio
        """
    ...
    async def preprocess_audio(self, file):
        ...
        Returns:
            dict: A dictionary containing
            the duration in seconds, sample rate, cleaned audio, and processed
        ...
        def sync_preprocess(file):
            try:
                logger.info(
                    f"[preprocess_audio] Processing audio file: {file}"
                )
                # 1️⃣  Basic file‑level sanity check (empty or unreadable file)
                if not os.path.exists(file) or os.path.getsize(file) == 0:
                    logger.warning(
                        f"[preprocess_audio] File {file} is empty or missing."
                    )
                    return {
                    ...
                    pre_emp_audio = np.append(
                        audio[0], audio[1:] - pre_emphasis * audio[:-1]
                    )
                    reduced_noise = nr.reduce_noise(y=pre_emp_audio, sr=sr)
                    processed_audio = self.save_processed_audio(
                        reduced_noise,
                        "processedAudio",
                ...
                logger.error(f"""Audio cleaning failed.
                    preprocess_audio function has error: {e}""")
                logger.error(traceback.format_exc())
                raise HTTPException(
                    status_code=500, detail="Audio cleaning failed"
                )
