        ...
        for attempt in range(max_retries):
            try:
                ...
                for _ in range(
                    0,
                    int(
                        self.sample_rate / self.chunk_size * self.clip_duration
                    ),
                ):
                ...
                if len(self.frames) == 0:
                    ...
                    audio_Saved = self.save_audio(
                        self.frames, "recording", file_path
                    )
                    audio_filename = audio_Saved["filename"]
                    audio_filepath = audio_Saved["filepath"]
                    website_audio = os.path.join(
                        audio_displayURL, audio_filename
                    )
                ...
                if attempt < max_retries - 1:
                    logger.info("Retrying recording in 5 seconds...")
                    ...
                    raise HTTPException(
                        status_code=500,
                        detail="Error in recording audio function",
                    )
