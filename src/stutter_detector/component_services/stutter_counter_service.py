...
class StutterCounterService:
    ...
    def repetitions_count(self, repetitions):
        ...
        Args:
            repetitions (list): A list of repetition
            occurrences, each containing start and end times.
        Returns:
        ...
        try:
            ...
            if num_repetitions > 0:
                ...
                self.stutter_count.append(f"""⚠️ You had {num_repetitions}
                    repetition(s) at: {time_string}.""")
    ...
    def prolongations_count(self, prolongations):
        ...
        Args:
            prolongations (list): A list of
            prolongation occurrences, each containing start and end times.
        Returns:
        ...
        try:
            ...
            if num_prolongations > 0:
                self.stutter_count.append(
                    f"⚠️ You had prolongations at these times:{prolongations}."
                )
            else:
                self.stutter_count.append(
                    "✅ Great! No prolongations detected."
                )
        except Exception as e:
            logger.error(f"Error in prolongations_count: {e}")
            logger.error(traceback.format_exc())
            self.stutter_count.append(
                "❌ Error generating prolongations count."
            )
        return self.stutter_count

    def blocks_count(self, blocks):
        """
        Counts and logs the number of alignment
        mismatch blocks in the provided list.
        Args:
            blocks (list): A list of block
            occurrences, each containing start and end times.
        Returns:
        ...
        try:
            ...
            mismatch_blocks = [
                b
                for b in blocks
                if b.get("type") == "alignment_mismatch_block"
            ]
            ...
            if num_blocks > 0:
                ...
                time_string = "; ".join(time_ranges)
                self.stutter_count.append(f"""⚠️ You had {num_blocks}
                    alignment mismatch block(s) at: {time_string}.""")
            else:
    ...
    def repeated_word_count(self, repeated_words):
        ...
        try:
            ...
            if num_repeated_words > 0:
                self.stutter_count.append(f"""⚠️ You had the
                    following repeated words {repeated_words}.""")
                self.stutter_count.append(
                    f"Repeated words: {', '.join(repeated_words)}"
                )

            else:
                self.stutter_count.append(
                    "✅ Great! No repeated words detected."
                )
        except Exception as e:
            logger.error(f"Error in repeated_word_count: {e}")
            logger.error(traceback.format_exc())
            self.stutter_count.append(
                "❌ Error generating repeated words count."
            )
        return self.stutter_count
    ...
    def fillers_count(self, fillers):
        ...
        try:
            ...
            if num_fillers > 0:
                self.stutter_count.append(
                    f"⚠️ You had the following fillers {fillers}."
                )
    ...
    def convert_count_to_string(
        self, fillers, repeated_words, blocks, prolongations, repetitions
    ):
