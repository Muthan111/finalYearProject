from src.stutter_detector.component_services.audio_analysis_service import (
...
class MfccPipeline:
    ...
    async def run_pipeline(self, audio_path, sr):
        ...
        try:
            mfcc_features = await self.audio_analysis_service.extractMFCC(
                audio_path, sr
            )
            if "error" in mfcc_features:
                return {"error": mfcc_features["error"]}

            return {"mfcc_features": mfcc_features["mfcc"], "sr": sr}
